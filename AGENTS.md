...
9. **Кодировка:** не передавать русские (не-ASCII) тексты через bash-heredoc,
   `python -c` и аргументы командной строки — shell на этой машине коверкает
   юникод (кириллица → `?`, эм-даш → артефакт `вЂ"`; именно так был испорчен
   `tools/magicIoTm/routes/projects.py`, починка — коммит `d42c2d09`).
   Файлы с кириллицей создавать и править только инструментом-редактором;
   коммит-сообщения — через `git commit -F <файл>` (файл сообщения тоже писать
   редактором); передачу файлов между командами — байтовым перенаправлением
   (`git show ... > файл`, `cp`), а не текстом в командной строке.
