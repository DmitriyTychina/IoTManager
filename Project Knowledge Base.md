...
**Внутренняя база знаний ассистента о проекте** · Обновлено: 2026-08-31 · Язык: русский

> Этот файл — рабочий конспект ассистента. Используется как основа для последующих задач по проекту. Дополняется по мере изучения кода.

---

# Проект: IoTManager — веб-инструмент прошивки IoT (tools/magicIoTm)
...
| Дата | Событие |
|------|---------|
| 2026-08-31 | Создание файла на основе полного аудита `tools/magicIoTm`. |
...
| 2026-09-02 | Архитектура `data_svelte`: каноническая папка данных проекта теперь **всегда в корне проекта** (`<проект>/data_svelte`), а не в `iotm/<платформа>/data_svelte`. Изменены `PrepareProject.py` (DATA_DIR), `app.py` (data_dir для сборки, get_fs_usage), `measure.py` (fs_used_dir). После успешной сборки в `iotm/` остаётся только одна папка с прошивками (`build.py::_prune_iotm`), остальные удаляются. Убран лишний шаг копирования data_svelte. |