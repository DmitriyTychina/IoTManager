...
- `_device_states` (ключ = базовое имя папки `<имя> <id>`, разделитель — пробел) —
  состояние-машина статуса устройства: `{state, fails, last_confirm}`. Пороги
  `FAILS_YELLOW=3`, `FAILS_RED=3`. Сопоставление через `_folder_key_candidates` учитывает
  и легаси-разделитель `_` (папки старого формата).

## Статус устройств (цветная точка): конечный автомат (app.py)

Статус НЕ зависит только от пинга — требуется подтверждение идентичности по имени и id
(папка устройства идентифицируется ключом `<имя>_<id>`). Чистая логика переходов — функция
`_next_state(cur, fails, confirmed, ping_ok)` (юнит-тесты в `test_device_status_fsm.py`).

- **🟢 Зелёный** — свежий multicast с совпадением `name`+`id` папки, ЛИБО успешный пинг +
  актуальный `settings.json` с совпадением `name`+`id`. Счётчик `fails` сбрасывается в 0.
- **⚪ Серый** — статус не подтверждён: пинг успешен, но multicast не приходил / не совпадает
  `name`+`id`, или данные неизвестны. Достижим из любого состояния при успешном пинге без подтверждения.
- **🟡 Жёлтый** — только ПОСЛЕ зелёного: 1..`FAILS_YELLOW` провалов пинга (multicast не обновлялся).
- **🔴 Красный** — только ПОСЛЕ жёлтого: более `FAILS_RED` провалов пинга.

Ключевые правила:
- Жёлтый/красный из серого недоступны — сначала устройство должно стать зелёным.
- Из жёлтого/красного повторное подтверждение (multicast или пинг+settings) сразу возвращает зелёный.
- Успешный пинг без подтверждения переводит зелёный/жёлтый/красный в серый.
- **Живой multicast авторитетен над settings.json**: при свежем multicast папка подтверждается
  только по нему, а пинг/settings НЕ выполняются (при разделяемом IP settings может вернуть имя
  другого устройства; после переименования остаются старые папки с тем же id).
- **После свежего multicast устройство не пингуется 70 сек** (`PING_SKIP_AFTER_MULTICAST=70`):
  multicast идёт каждые 60 сек и сам подтверждает живость и идентичность.
- **Имена устройств в вебе не изменяются**: `dispDeviceName` выводит имя без замены `_` на пробелы
  (в отличие от `dispName`, который используется для проектов).

Источник идентичности при отсутствии multicast — `ws_client.fetch_settings(host)` (команда `/list|`,
блок `settin`). Пинг (`_mark_device_seen`) НЕ обновляет `last_seen` multicast и зелёный не даёт.

Пауза при скачивании: `_device_busy_fetching(ip)` исключает занятый IP из `_ping_cycle`, а
`_fetch_settings_identity` дополнительно возвращает `None` для занятого IP — во время `fetch`
(RAM/FS) пинг и WS-запросы к устройству не выполняются.

Гарантия одного зелёного на IP: `_enforce_single_green(ip, keys, confirmed_here)` — если несколько
папок с одним IP оказались зелёными, зелёным остаётся только подтверждённая в текущем цикле,
остальные переводятся в серый (защита от дублей/коллизий ключей).
